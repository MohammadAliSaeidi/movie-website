...
export default function MovieCard({
	...
	const genres = Genre.split(", ").map((genre) => (
		<div className="px-2 py-1 flex-nowrap min-w-fit bg-slate-950/[0.1] dark:bg-slate-50/[.10] text-xs font-light w-fit rounded-full">
			{genre}
		</div>
	));

	return (
		<div className="flex flex-1 select-none flex-col relative z-10 w-64 sm:w-56 md:w-54 lg:w-48 cursor-pointer overflow-hidden card hover:scale-105 ease-out transition-all duration-300 hover:shadow-black/15 hover:shadow-xl shadow-sm">
			<ImageWithSkeleton
